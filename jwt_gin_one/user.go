...
func (us *Users) Add(username string, desc string) *User {
	...
	fmt.Println(query)
	db, err := sql.Open("sqlite3", "mytest")
...
func IdentityHandler(c *gin.Context) interface{} {
	...
	return &User{
		UserName: claims[identityKey0].(string),
	}
}
...
func main() {
	...
	//var users *Users
	//var user *User
	users = InitUsers("mytest")
	user = users.Add("nice", "content")
	fmt.Println(user)
}